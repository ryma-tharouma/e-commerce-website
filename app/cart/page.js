  ...
  return (
    <div className="min-h-screen flex flex-col justify-between bg-gray-100">
      ...
      <header className="bg-white shadow-md py-2">
        ...
        <div className="container mx-auto flex justify-between items-center px-6 text-sm text-gray-700">
          ...
          <div className="flex space-x-4">
            ...
            <Link href="/signin" className="hover:text-[#D4AF37]">Sign in &gt;</Link>
        ...
        <div className="container mx-auto flex justify-between items-center">
          <nav className="container mx-auto flex justify-center space-x-6 font-semibold py-2">
            {["recent", "fineart", "antiques", "jewelry", "library", "about"].map((item) => (
              <Link key={item} href={`/${item}`} className="text-xs font-[Georgia] hover:text-[#D4AF37]">
      ...
      <main className="flex flex-col items-center justify-center flex-grow min-h-[50vh] py-10">
        ...
        <button
          onClick={() => addToCart({ id: 1, name: "Antique Vase", price: 150, image: "/vase1.jpg" })}
        ...
        {cart.length === 0 ? (
          <>
            <p className="text-gray-600 text-lg mb-8">Your shopping bag is currently empty.</p>
            <Link href="/">
      ...
      <footer className="bg-white shadow-md py-6 text-gray-700">
        <div className="container mx-auto grid grid-cols-2 md:grid-cols-4 gap-6 px-6 text-sm">
          {[
            { title: "Shop", links: ["Recent Acquisitions", "Antiques", "Jewelry", "Fine Art"] },
            { title: "Can We Help?", links: ["Customer Care", "FAQs", "Design Services", "Contact Us"] },
            { title: "Our Company", links: ["Buy with Confidence", "Events", "Sell to Us", "Careers"] },
          ].map((section, index) => (
            <div key={index}>
              <h3 className="font-semibold">{section.title}</h3>
              <ul>
                {section.links.map((link, i) => (
                  <li key={i} className="flex items-center hover:text-[#D4AF37] cursor-pointer">
                    {link}
                  </li>
                ))}
              </ul>
        ...
        <div className="text-center text-xs mt-6">© 2025 M.S. Rau | Privacy | Site Map</div>
      </footer>
    </div>
  );
}
