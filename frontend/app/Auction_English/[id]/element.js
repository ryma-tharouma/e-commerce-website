...
export default function English_Auction_Item() {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    fetch(`http://127.0.0.1:8000/Auction_English/auctions/${id}/`) // Remplace par l'ID réel
      .then((response) => {
  ...
  const handleBid = async () => {
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/Auction_English/auctions/${id}/bid/`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        const errro = await response.json();
      ...
      <div className="px-4 py-20 col-span-4 space-y-4 ">
        ...
        {!isAuctionEnded && (
          <div className=" rounded-lg">
            ...
            <button
              className="mt-4 w-full bg-yellow-600 text-white py-2 text-sm font-semibold  hover:bg-yellow-700"
              onClick={handleBid}
            >
              Place Bid{" "}
            </button>
            ...
            />
            {error ? (
              <p className="text-center font-[Georgia] text-red-500 m-2">
                {error.message}
              </p>
            ) : (
              <></>
